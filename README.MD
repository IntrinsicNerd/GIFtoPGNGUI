...
Make sure the board is as square as possible and you crop out anything bordering the board. For now, only works on games that start on turn 1(i.e. the gif starts on whites first move, not an initialized board state or a title screen). This will be fixed in the future
